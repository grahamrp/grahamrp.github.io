...
One way of refining the model to get the *steady* climbers is to use its R^2^ value in concert with the slope. The R^2^ value measures how well the model fits the data, taking a value from 0 to 1. Models with a high r-squared will fit the data closely, and in this case will look more like a straight line. For example, Leeds Trinity has a steep upward slope (`r filter(scores_per_year, inst == 'Leeds Trinity')$score_per_year` points per year) but doesn't look much like a straight line (**R^2^ `r round(filter(scores_per_year, inst == 'Leeds Trinity')$r.squared, 2)`**) whereas Falmouth has a steep slope (`r filter(scores_per_year, inst == 'Falmouth')$score_per_year` points per year) but also looks more like a straight line (**R^2^ `r round(filter(scores_per_year, inst == 'Falmouth')$r.squared, 2)`**).
